...
# =========================================
print(
    f"Processing time range: {START_DATE.date()} -> {END_DATE.date()} "
...
if os.path.exists(df_power_out_path):
    print(f"Power weights file already exists -> loading:\n{df_power_out_path}", flush=True)
    ...
    print("No power weights file found -> creating new one", flush=True)
...
for i, station in enumerate(station_list, start=1):
    ...
    print(
        f"    Time span: {df_station_weather['Datetime'].min().date()} -> "
    ...
    if lost_weather > 0:
        print(f"    Warning: Dropped {lost_weather} MeteoSwiss hours (missing data)")
